...
/**
 * The Atom is a class that implements an atom and perform all the task the
...
public class Atom extends XRDcat {
  ...
  static final int xcoordNumber = 1, ycoordNumber = 2, zcoordNumber = 3;
   ...
   */
  public Atom(XRDcat obj, String alabel) {
    super(obj, alabel);
    initXRD();
  }
  ...
  public XRDcat getCopy(XRDcat parent) {
    Atom atom_copy = (Atom) super.getCopy(parent);
    if (atom_copy != null)
...
//      System.out.println("SetField, starting symbol: " + getAtomSymbol());
      stringField[0] = checkAtomSymbol(checkOxidationNumber(getAtomSymbol()));
...
//    System.out.println("Update starting symbol: " + oldSymbol);
    stringField[0] = checkAtomSymbol(checkOxidationNumber(oldSymbol));
...
//      System.out.println("Starting symbol as label: " + alabel);
      stringField[0] = checkAtomSymbol(checkOxidationNumber(alabel));
...
//      stringField[0] = alabel;
  ...
  /**
   * Return the radius of this atom in Angstrom
   *
   * @return the actual radius of the atom
   */
  ...
  /**
   * Set the radius of this atom in Angstrom. Normally the default radius is read from
   * the atominfo.cif file.
   *
   * @param rad the radius to be set
   */
  public void setAtomRadius(double rad) {
    radius = rad;
  }
  ...
  /**
   * Set the weight of this atom. Normally the default weight is read from
   * the atominfo.cif file.
   *
   * @param wgt the weight to be set.
   */
  public void setAtomWeight(double wgt) {
    weight = wgt;
  }
   ...
   */
  public int checkOxidationNumber() {
    return retrieveOxidationNumber(getAtomSymbol());
  }
   ...
   */
  public String checkAtomSymbol(String symbol) {
    if (symbol.length() < 1)
      return tryFromLabel();
  ...
  public void refreshPositions(boolean cartesianAlso) {
	...
	public void setAnisotropicBfactors() {
		...
		if (Constants.testing)
			System.out.println("Atom " + getSiteLabel() + ": symmetry matrix B factors: B" + (symmetryMatrixIndex + 1));
	}
  ...
  public boolean equalsByDistance(Object obj) {
    ...
    if (obj instanceof Atom) {
      Atom ato2 = (Atom) obj;
      if (ato2.getAtomSymbol().equalsIgnoreCase(getAtomSymbol())) {
  ...
  public void transformAbsoluteCoordToLocal(Coordinates coordFirst) {
    ...
    if (parent instanceof Fragment) {
      ...
      coordFirst = ((Fragment) parent).calcRelativeCoords(coordFirst);
    }
  }
...
//        Double.valueOf(getQuantity()).doubleValue());
  ...
  public void computeQuantityFromOccupancy() {
    int newSitePosNumber = getSiteMultiplicity();
    setQuantity(Fmt.format(getOccupancy().getValueD() * newSitePosNumber));
  }
...
//		double halfRadius = radius / 2.0;
    return tooShortDistance(coordFirst, coordNext, radius);
  }
  ...
  public double getSiteWeight() {
    refreshPositions(false);
    return getSiteMultiplicity() * getOccupancy().getValueD() * weight;
  }

  /**
   * Gets the total number of atoms of this object in the cell, equal to the quantity value.
   *
   * @return the total number of atoms of this atom site.
   */
...
//    if (rad == null || rad.isNeutron() || rad.isElectron())
//      return getSiteNumber() * rad.getAbsorption(this);
	  if (rad.isNeutron())
      ...
      return getSiteAbsorption(Constants.ENERGY_LAMBDA / rad.getMeanRadiationWavelength() * 0.001);
  }
...
//    System.out.println("Atom " + getAtomSymbol() + ", atomic number: " + atomListNumber);
...
//    System.out.println("Atom " + getAtomSymbol() + ", isotopic number: " + isotopeListNumber);
...
//    System.out.println("Atom " + getAtomSymbol() + ", atom number: " + atomNumber);
    return atomNumber;
  }
  ...
  public double[] scatfactorNoDispersion(double dspacing, Radiation rad) {
...
//        cellVolume2 *= (cellVolume2 / 2.);
  ...
  public void loadAtomProperties() {
    ...
    if (loadedSymbol == null || !symbol.equalsIgnoreCase(loadedSymbol)) {
      ...
      oxidationNumber = checkOxidationNumber();
   ...
   */
  public boolean shareSiteWith(Atom anatom) {
    boolean share = false;
    boolean flagx1;
    double x[] = new double[3];
   ...
   */
  public void boundAllParametersTo(Atom anatom) {
  ...
  /**
   * Set the atomic weight to the specified one.
   *
   * @param weight the weight to be set for this atom
   */
  ...
  /**
   * Sets the atomic radius in Angstrom for this atom.
   *
   * @param radius the atomic radius to be set.
   */
  ...
  public String getInformationString() {
    ...
    astring.append(" ");
    astring.append(getRadius());
    astring.append(" ");
    astring.append(getWeight());
