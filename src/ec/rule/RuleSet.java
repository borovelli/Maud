...
// reinitialize the array of rules
  ...
  public void mutateRules(final EvolutionState state, final int thread) {
    ...
    while (state.random[thread].nextBoolean(constraints().p_del)) {
      removeRandomRule(state, thread);
    }
    while (state.random[thread].nextBoolean(constraints().p_add)) {
...
// if there's not enough place to store the new rules, increase space
  ...
  /**
   ...
   Comment: This function appends the splitted rulesets to the existing rulesets existing
   in the sets parameter.  This feature is used in the RuleCrossoverPipeline class.
   */
