...
// this function.
    ...
    try {
      ...
      mailbox = new IslandExchangeMailbox(state, clientPort, Integer.parseInt(fromServer.readLine()),
              Integer.parseInt(fromServer.readLine()), ownId);
    ...
    if ((state.generation >= offset) &&
      ...
      for (int x = 0; x < number_of_destination_islands; x++) {
        ...
        if (running[x]) {
          ...
          state.output.message("Sending the emigrants to island " + outgoingIds[x]);
...
// create the ServerSocket to listen to incoming messages
    try {
      ...
      state.output.fatal("Could not start server for incoming messages.");
    }
...
// waiting for messages on a particular one.
    for (int x = 0; x < n_incoming; x++) {
      try {
        ...
        PrintWriter printer = new PrintWriter(inSockets[x].getOutputStream(), true);
