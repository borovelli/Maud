...
// gather timings
    ...
    for (int x = 0; x < state.population.subpops.length; x++) {
      ...
      for (int y = 0; y < state.population.subpops[x].individuals.length; y++) {
        ...
        if (doFull) {
          // mean size for population
          meanSize += state.population.subpops[x].individuals[y].size();
        }
      }

      // compute fitness stats
      meanFitness /= state.population.subpops[x].individuals.length;
      meanSize /= state.population.subpops[x].individuals.length;
      ...
      if (doFull) {
        state.output.print("" + (double) meanSize + " " + (double) (best_i[x].size()) + " " +
