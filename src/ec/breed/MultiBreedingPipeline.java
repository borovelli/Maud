...
// Now check for nonzero probability (we know it's positive)
    if (total == 0.0)
      state.output.fatal("MultiBreedingPipeline must not all be zero probability", base);
...
/* -- no longer necessary
    public void prepareToProduce(final EvolutionState s,
				 ...
				 final int thread)
	{
	// make sure that we know how many we can produce --
	// we've set typicalIndsProduced() to compute this for us
	// and store it.
	typicalIndsProduced();
