...
// allocate some of the arrays
    ...
    for (int i = 0; i < numsubpops; i++) {
      ...
      exchangeInformation[i].immigrantsSelectionMethod.setup(state, base);
      ...
      exchangeInformation[i].indsToDieSelectionMethod.setup(state, base);
    ...
    for (int i = 0; i < exchangeInformation.length; i++) {
      ...
      if ((state.generation >= exchangeInformation[i].offset) &&
        ...
        for (int x = 0; x < exchangeInformation[i].numDest; x++) {
          ...
          state.output.message("Sending the emigrants from subpopulation " +
                  i + " to subpopulation " +
