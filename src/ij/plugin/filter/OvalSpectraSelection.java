...
//		OvalSpectraPlot p = new OvalSpectraPlot(imp, radius);
          new SpectraProfilePlot(imp, false).createWindow(imp, usableRectangle);
      }
    }).start();
  }

  public void checkExistingRoi() {
    Roi roi_rect = imp.getRoi();
    if (roi_rect == null)
      usableRectangle = null;
    else {
    int roiType = roi_rect.getType();
    if (roiType == Roi.RECTANGLE)
      usableRectangle = roi_rect.getBounds();
    else {
      usableRectangle = null;
    }
    }
