...
public class IntegerVectorSpecies extends VectorSpecies {
  ...
  public void setup(final EvolutionState state, final Parameter base) {
    ...
    minGene = state.parameters.getLongWithDefault(base.push(P_MINGENE), null, 0);
    maxGene = state.parameters.getLong(base.push(P_MAXGENE), null, minGene);
    if (maxGene < minGene)
    ...
    if (state.parameters.exists(base.push(P_MAXGENE).push("0"), null)) {
      ...
      for (int x = 0; x < genomeSize; x++) {
        ...
        if (!state.parameters.exists(base.push(P_MINGENE).push("" + x), null)) {
          ...
          minGenes[x] = state.parameters.getLongWithDefault(base.push(P_MINGENE).push("" + x), null, minGene);
        ...
        if (!state.parameters.exists(base.push(P_MAXGENE).push("" + x), null)) {
          ...
          maxGenes[x] = state.parameters.getLongWithDefault(base.push(P_MAXGENE).push("" + x), null, maxGene);
